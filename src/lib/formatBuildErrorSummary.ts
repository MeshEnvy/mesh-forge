...
export function formatBuildErrorSummary(summary: string | undefined): string {
  ...
  if (summary.includes("Unexpected inputs provided")) {
    return (
      "MeshForge’s own GitHub workflow didn’t accept the build request (inputs out of sync). " +
      "That’s on the MeshForge service, not your firmware. Retry won’t help until that workflow YAML is updated."
    )
  }
...
export function buildFailurePresentation(summary: string | undefined): {
  ...
  if (summary.includes("Unexpected inputs provided")) {
    return {
      ...
      body:
        "Something on the MeshForge side didn’t line up with GitHub. It’s not a signal that your repo is broken. " +
